...
from .compat import is_py3, xrange
...
class CR1000(object):
    ...
    def from_url(
        ...
        return cls(
            link, dest_addr, dest, src_addr, src, security_code
    ...
    def getfile(self, filename):
        ...
        while True:
            ...
            cmd = self.pakbus.get_fileupload_cmd(
                filename, offset=offset, closeflag=0x00, transac_id=transac_id
            )
            transac_id = cmd[1]
    ...
    def table_fields(self, tablename):
        ...
        fields = [
            i["Fields"] for i in self.table_def if i["Header"]["TableName"] == tablename
        ]
    ...
    def _collect_data(self, tablename, start_date=None, stop_date=None):
        ...
        cmd = self.pakbus.get_collectdata_cmd(tablenbr, tabledefsig, mode, p1, p2)
        hdr, msg, send_time = self.send_wait(cmd)
        more = True
    ...
    def get_value(self, tablename, fieldname, swath=None, type_=None):
        ...
        hdr, msg, send_time = self.send_wait(
            self.pakbus.get_values_cmd(tablename, type_, fieldname, swath=swath)
        )
    ...
    def set_value(self, tablename, fieldname, value, type_=None):
        ...
        hdr, msg, send_time = self.send_wait(
            self.pakbus.set_values_cmd(tablename, type_, fieldname, value)
        )
