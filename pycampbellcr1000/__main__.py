...
from .utils import ListDict, csv_to_dict
...
def getfile_cmd(args, device):
    """Getfile command."""
    args.output.write(device.getfile(args.filename))
...
def getdata_cmd(args, device, header=True, exclude_first=False):
    ...
    for i, records in enumerate(generator):
        ...
        args.output.write("%s" % records.to_csv(delimiter=args.delim, header=header))
        if header:
            header = False
...
def get_cmd_parser(cmd, subparsers, help, func):
    ...
    parser = subparsers.add_parser(
        cmd, help=help, description=help, formatter_class=formatter_class
    )
    parser.add_argument(
        "--timeout", default=10.0, type=float, help="Connection link timeout"
    )
    parser.add_argument(
        "--src_addr", default=None, type=int, help="Source physical address"
    )
    parser.add_argument("--src", default=0x802, type=int, help="Source node ID")
    parser.add_argument(
        "--dest_addr", default=None, type=int, help="Destination physical address"
    )
    parser.add_argument("--dest", default=0x001, type=int, help="Destination node ID")
    parser.add_argument(
        "--code", default=0x0000, type=int, help="Datalogger security code"
    )
    parser.add_argument(
        "--debug", action="store_true", default=False, help="Display log"
    )
    parser.add_argument(
        "url",
        action="store",
...
def main():
    ...
    parser.add_argument(
        "--debug", action="store_true", default=False, help="Display log"
    )  # needed in case not command is given

    subparsers = parser.add_subparsers(title="The PyCR1000 commands")
    # gettime command
    subparser = get_cmd_parser(
        "gettime",
        subparsers,
        help="Print the current datetime of the" " datalogger.",
        func=gettime_cmd,
    )
    ...
    subparser = get_cmd_parser(
        "settime",
        subparsers,
        help="Set the given datetime argument on the" " datalogger.",
        func=settime_cmd,
    )
    subparser.add_argument(
        "datetime",
        help="The chosen datetime value. "
    ...
    subparser.add_argument(
        "--delim", action="store", default=",", help="CSV char delimiter"
    )
    ...
    subparser = get_cmd_parser(
        "getfile",
        subparsers,
        help="Get the file content from the " "datalogger.",
        func=getfile_cmd,
    )
    subparser.add_argument(
        "filename", action="store", help="Filename to be downloaded."
    )
    subparser.add_argument(
        "output",
        action="store",
    ...
    subparser.add_argument(
        "table", action="store", help="The table name used for data collection"
    )
    subparser.add_argument(
        "output",
        action="store",
    ...
    subparser.add_argument(
        "--start", help="The beginning datetime record " '(like : "%s")' % NOW
    )
    subparser.add_argument(
        "--stop", help="The stopping datetime record " '(like : "%s")' % NOW
    )
    subparser.add_argument(
        "--delim", action="store", default=",", help="CSV char delimiter"
    )
    ...
    subparser = get_cmd_parser(
        "update",
        subparsers,
        help="Update CSV database records by getting " "automatically new records.",
        func=update_cmd,
    )
    subparser.add_argument(
        "table", action="store", help="The table name used for data collection"
