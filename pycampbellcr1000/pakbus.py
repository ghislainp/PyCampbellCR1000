...
from .compat import bytes, chr, is_py3, is_text, ord
from .exceptions import DeliveryFailureException
from .logger import LOGGER
from .utils import Singleton, bytes_to_hex, nsec_to_time
...
class PakBus(object):
    ...
    def wait_packet(self, transac_id=None):
        ...
        LOGGER.info("src, SrcNodeId = <%x, %x>" % (self.src, hdr["SrcNodeId"]))
        LOGGER.info("dest, DstNodeId = <%x, %x>" % (self.dest, hdr["DstNodeId"]))
    ...
    def pack_header(self, hi_proto, exp_more=0x2, link_state=None, hops=0x0):
        ...
        hdr = struct.pack(
            ...
            (exp_more & 0x3) << 14 | (priority & 0x3) << 12 | (self.src_addr & 0xFFF),
    ...
    def decode_packet(self, data):
        ...
        LOGGER.info(
            "HiProtoCode, MsgType = <%x, %x>" % (hdr["HiProtoCode"], msg["MsgType"])
        )
        ...
        # BMP5 Application Packets
            ...
            LOGGER.error(
                "No implementation for <(%r, %r)> packet type"
    ...
    def unpack_getsettings_response(self, msg):
        ...
        msg["Settings"] = []
        if msg["Outcome"] == 0x01:
            values, size = self.decode_bin(
                ["UInt2", "Byte", "Byte", "Byte"], msg["raw"][offset:]
            )
            ...
            while offset < len(msg["raw"]):
                # Get setting ID
                [SettingId], size = self.decode_bin(["UInt2"], msg["raw"][offset:])
                offset += size
    ...
    def get_collectdata_cmd(self, tablenbr, tabledefsig, mode=0x04, p1=0, p2=0):
        ...
        msg = self.encode_bin(
            ["Byte", "Byte", "UInt2", "Byte"],
            [0x09, transac_id, self.security_code, mode],
        )
    ...
    def get_clock_cmd(self, adjustment=(0, 0)):
        ...
        hdr = self.pack_header(0x1)
        msg = self.encode_bin(
            ["Byte", "Byte", "UInt2", "NSec"],
            [0x17, transac_id, self.security_code, adjustment],
        )
        return b"".join((hdr, msg)), transac_id
    ...
    def get_getprogstat_cmd(self):
        ...
        hdr = self.pack_header(0x1)
        msg = self.encode_bin(
            ["Byte", "Byte", "UInt2"], [0x18, transac_id, self.security_code]
        )
        return b"".join((hdr, msg)), transac_id
    ...
    def get_filedownload_cmd(
        self, filename, data, offset=0x00000000, closeflag=0x00, transac_id=None
    ):
        ...
        """
        raise NotImplementedError("Filedownload transaction is not implemented" " yet")

    def unpack_filedownload_response(self, msg):
        """Unpack Filedownload Response packet."""
        raise NotImplementedError("Filedownload transaction is not implemented" " yet")

    def get_fileupload_cmd(
        self, filename, offset=0x00000000, swath=0x0200, closeflag=0x01, transac_id=None
    ):
            ...
            # Handle complete records (standard case)
                ...
                for n in range(frag["NbrOfRecs"]):
                    ...
                    for field in fields:
                        ...
                        if fieldtype == "ASCII":
                            values, size = self.decode_bin(
                                [fieldtype], raw[offset:], dimension
                            )
                            record["Fields"][fieldname] = values[0]
                        else:
                            values, size = self.decode_bin(
                                dimension * [fieldtype], raw[offset:]
                            )
                            if dimension > 1:
    ...
    def get_values_cmd(self, table_name, type_, field_name, swath=1):
        ...
        hdr = self.pack_header(0x1)
        msg = self.encode_bin(
            ["Byte", "Byte", "UInt2", "ASCIIZ", "Byte", "ASCIIZ", "UInt2"],
            [
                0x1A,
                transac_id,
                self.security_code,
                table_name,
                self.DATATYPE[type_]["code"],
                field_name,
                swath,
            ],
        )
    ...
    def set_values_cmd(self, table_name, type_, field_name, value):
        ...
        if isinstance(
            value, list
        ):  # could be a collection.Sequence but string must be detected. Need six... which is an external dep
            swath = len(value)
            type_ = [type_] * swath
        else:
            value = [value]
        ...
        hdr = self.pack_header(0x1)
        msg = self.encode_bin(
            ["Byte", "Byte", "UInt2", "ASCIIZ", "Byte", "ASCIIZ", "UInt2"] + type_,
            [
                0x1B,
                transac_id,
                self.security_code,
                table_name,
                self.DATATYPE[type_[0]]["code"],
                field_name,
                swath,
            ]
            + value,
