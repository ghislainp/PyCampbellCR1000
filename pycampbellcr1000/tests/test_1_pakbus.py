...
def test_get_collectdata_cmd():
    ...
    assert (
        cmd == "A0 01 98 02 10 01 08 02 09 03 00 00 07 00 02 9E A7 2A"
...
def test_get_fileupload_cmd():
    ...
    assert (
        cmd == "A0 01 98 02 10 01 08 02 1D 06 00 00 46 69 6C 65 6E 61 6D"
...
def test_parse_filedir():
    pakbus = PakBus(FakeLink())
    data = (
        "01 43 50 55 3A 00 00 07 6E 00 00 00 43 50 55 3A 74 65 6D"
...
def test_parse_collectdata():
    ...
    tabledef = pakbus.parse_tabledef(hex_to_bytes(TABLEDEF))
    raw = (
        "00 02 00 01 5B DC 00 06 2A 72 AB 30 00 00 00 00 45 51 13"
...
def test_getprogstat_response():
    ...
    assert (
        msg["Stats"]["CompResult"] == b"CPU:CR1000_LABO.CR1 -- Compiled "
        b"in PipelineMode.\r\n"
    )
...
def test_collectdata_response():
    pakbus = PakBus(FakeLink())
    packet = (
        "A8 02 10 01 18 02 00 01 9D 06 00 00 00 00 00 01 53 74 61"
